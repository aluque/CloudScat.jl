...
function run()
    ...
    for prof in profiles
        run_profile(prof, "Hale.dat")
    end

    run_profile("Tropical.dat", "WarrenBrandt.dat")
end


function run_profile(fname, refindex)
    ...
    c1 = Cylinder(7 * co.kilo,  15 * co.kilo, 0, 0, 200 * co.kilo)
    cloud = c1
    ...
    refindexbasename = splitext(refindex)[1]
    ...
    CloudScat.main(params, world, observers,
                   saveto="$(basename)_$(profbasename)_$(refindexbasename).h5")
end
